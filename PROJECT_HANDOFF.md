...
- Target shared asset: `/public/rythero-og.jpg`, 1200×630 JPEG.
- Base layout already references `/rythero-og.jpg` with `og:image`, secure URL, type, width, height and alt metadata.
...
- Verify the binary asset is present on the current main branch and publicly reachable after Cloudflare deployment before calling the social card fully live.
...
3. Verify `/rythero-og.jpg` exists on current main, loads publicly after deployment, and test social sharing previews before launch.
